...
class Response {
    ...
    public function send_loot($itens, $msg) {
        global $userDetails;
        $imgUrl = GAME_URL . "/Imagens/Backgrounds/realizacao2.php?m=" . base64_encode($msg) . "&id=" . $userDetails->tripulacao["id"];
        echo $this->share_msg("<img style='width:100%' src='$imgUrl' />", $imgUrl);
    }

    public function send_conquista_pers($pers, $msg) {
        global $userDetails;
        $imgUrl = GAME_URL . "/Imagens/Backgrounds/realizacao.php?cod=" . $pers["cod"] . "&id=" . $userDetails->tripulacao["id"] . "&m=" . base64_encode($msg);
        echo $this->share_msg("<img style='width:100%' src='$imgUrl' />", $imgUrl);
    }
    ...
    public function share_msg($msg, $urlToShare = "") {
      if ($urlToShare == '')
        $urlToShare = GAME_URL;
